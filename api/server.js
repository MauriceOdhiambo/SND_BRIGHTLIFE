...
function buildProfessionalPdf_(sections, meta){
  const PAGE_W=595, PAGE_H=842, LEFT=42, RIGHT=553, TOP=744, BOTTOM=52;
  const objects=[]; const kids=[]; const fontObj=3, boldFontObj=4;
  ...
  function addPage(content){
    ...
    objects[pageObj]=`<< /Type /Page /Parent 2 0 R /MediaBox [0 0 ${PAGE_W} ${PAGE_H}] /Resources << /Font << /F1 ${fontObj} 0 R /F2 ${boldFontObj} 0 R >> >> /Contents ${contentObj} 0 R >>`;
    kids.push(`${pageObj} 0 R`);
  }

  function header(pageNo){
    const c=[];
    c.push('q'); c.push(pdfColor_(7,27,49)+' rg'); c.push(`0 ${PAGE_H-82} ${PAGE_W} 82 re f`); c.push('Q');
    c.push('q'); c.push(pdfColor_(108,60,225)+' rg'); c.push(`0 ${PAGE_H-86} ${PAGE_W} 4 re f`); c.push('Q');
    c.push('q'); c.push(pdfColor_(255,255,255)+' rg'); c.push('34 754 48 48 re f'); c.push('Q');
    c.push('q'); c.push(pdfColor_(31,160,65)+' RG 1.8 w 41 761 34 34 re S');
    c.push(pdfColor_(34,145,203)+' rg'); c.push('55 765 m 50 774 54 785 61 787 c 68 785 70 774 64 765 c 61 761 58 758 55 765 f');
    c.push(pdfColor_(31,160,65)+' rg'); c.push('56 781 m 50 786 51 794 57 797 c 63 793 64 787 56 781 f');
    c.push('Q');
    c.push('BT /F2 15 Tf 1 1 1 rg 94 780 Td ('+pdfEscapeText_(meta.organization||'SND BRIGHTLIFE CBO')+') Tj ET');
    c.push('BT /F2 9 Tf 0.82 0.86 0.91 rg 94 762 Td ('+pdfEscapeText_(meta.reportTitle||'PROFESSIONAL REPORT')+') Tj ET');
    if(meta.memberCode || meta.memberName){
      c.push('BT /F1 8 Tf 0.78 0.83 0.89 rg 94 748 Td ('+pdfEscapeText_((meta.memberCode||'')+'  |  '+(meta.memberName||'')) +') Tj ET');
    }
    c.push('BT /F1 7 Tf 0.45 0.50 0.56 rg 42 30 Td (SND Brightlife CBO  |  Confidential  |  '+pdfEscapeText_(meta.generated||'')+') Tj ET');
    c.push('BT /F1 7 Tf 0.45 0.50 0.56 rg 505 30 Td ('+pageNo+' / '+meta.pageCount+') Tj ET');
    return c;
  }

  const rows=[];
  sections.forEach(function(sec){ rows.push({type:'section',text:sec.title}); (sec.rows||[]).forEach(function(row){ rows.push(row); }); rows.push({type:'space'}); });
  let pages=[], current=header(1), y=TOP-8;
  function newPage(){ if(current&&current.length) pages.push(current); current=header(pages.length+1); y=TOP-8; }
  function ensure(h){ if(y-h<BOTTOM) newPage(); }
  function textRow(text,size,bold,color,indent,leading){
    const lines=pdfWrapLines_(text,size<=8?105:size<=10?92:76), lead=leading||Math.max(10,Math.round(size*1.45));
    lines.forEach(function(line){ ensure(lead+2); current.push(`BT /F${bold?2:1} ${size} Tf ${color||'0.12 0.15 0.20'} rg ${indent||0} ${y} Td (${pdfEscapeText_(line)}) Tj ET`); y-=lead; });
  }
  function table(row){
    const cols=row.columns||[], data=row.data||[];
    if(!cols.length) return;
    const widths=row.widths || (cols.length===4?[78,170,82,139]:cols.length===5?[62,112,118,92,127]:cols.length===6?[62,82,128,88,82,69]:cols.length===7?[55,82,105,76,72,62,59]:Array(cols.length).fill((RIGHT-LEFT)/cols.length));
    const sum=widths.reduce((a,b)=>a+b,0), scale=(RIGHT-LEFT)/sum, cw=widths.map(w=>w*scale), pad=6, font=6.6, lead=8.5;
    function cellLines(v,w){ return pdfWrapLines_(String(v==null?'—':v),Math.max(7,Math.floor((w-pad*2)/3.25))); }
    function drawHeader(){
      const h=22; ensure(h+4); current.push('q'); current.push(pdfColor_(232,236,243)+' rg'); current.push(`${LEFT} ${y-h} ${RIGHT-LEFT} ${h} re f`); current.push('Q');
      let x=LEFT; cols.forEach(function(col,i){ current.push(`BT /F2 ${font} Tf 0.20 0.26 0.34 rg ${x+pad} ${y-14} Td (${pdfEscapeText_(String(col))}) Tj ET`); x+=cw[i]; }); y-=h;
    }
    drawHeader();
    data.forEach(function(r){
      const lines=r.map(function(v,i){return cellLines(v,cw[i]);});
      const maxLines=Math.max.apply(null,lines.map(function(x){return x.length;}));
      const h=Math.max(18,maxLines*lead+7);
      if(y-h<BOTTOM){ newPage(); drawHeader(); }
      if((data.indexOf(r)%2)===0){ current.push('q'); current.push(pdfColor_(249,250,252)+' rg'); current.push(`${LEFT} ${y-h} ${RIGHT-LEFT} ${h} re f`); current.push('Q'); }
      let x=LEFT;
      lines.forEach(function(ls,i){ ls.forEach(function(line,j){ current.push(`BT /F1 ${font} Tf 0.20 0.24 0.30 rg ${x+pad} ${y-12-(j*lead)} Td (${pdfEscapeText_(line)}) Tj ET`); }); x+=cw[i]; });
      y-=h;
    });
    y-=7;
  }
  rows.forEach(function(row){
    if(row.type==='space'){ y-=6; return; }
    if(row.type==='section'){ ensure(34); current.push('q'); current.push(pdfColor_(246,243,255)+' rg'); current.push(`${LEFT-2} ${y-22} ${RIGHT-LEFT+4} 25 re f`); current.push('Q'); textRow(String(row.text||'').toUpperCase(),10,true,pdfColor_(83,42,153),8,13); y-=4; return; }
    if(row.type==='identity'){
      const items=row.items||[], gap=8, w=(RIGHT-LEFT-gap*Math.max(0,items.length-1))/Math.max(items.length,1); ensure(64);
      items.forEach(function(item,i){ const x=LEFT+i*(w+gap); current.push('q'); current.push(pdfColor_(248,250,252)+' rg'); current.push(`${x} ${y-52} ${w} 52 re f`); current.push('Q'); current.push(`BT /F1 7 Tf 0.35 0.40 0.48 rg ${x+9} ${y-16} Td (${pdfEscapeText_(item.label||'')}) Tj ET`); current.push(`BT /F2 9 Tf 0.08 0.10 0.14 rg ${x+9} ${y-36} Td (${pdfEscapeText_(item.value||'—')}) Tj ET`); }); y-=64; return;
    }
    if(row.type==='kpis'){
      const values=row.values||[], gap=8, w=(RIGHT-LEFT-gap*Math.max(0,values.length-1))/Math.max(values.length,1); ensure(70);
      values.forEach(function(k,i){ const x=LEFT+i*(w+gap); current.push('q'); current.push(pdfColor_(248,250,252)+' rg'); current.push(`${x} ${y-58} ${w} 60 re f`); current.push('Q'); current.push(`BT /F1 7 Tf 0.35 0.40 0.48 rg ${x+8} ${y-15} Td (${pdfEscapeText_(k.label)}) Tj ET`); current.push(`BT /F2 12 Tf 0.08 0.10 0.14 rg ${x+8} ${y-36} Td (${pdfEscapeText_(k.value)}) Tj ET`); }); y-=72; return;
    }
    if(row.type==='table'){ table(row); return; }
  ...
  pages=pages.map(function(c,i){ const idx=c.findIndex(function(x){return x.indexOf('505 30 Td (')>=0;}); if(idx>=0)c[idx]=`BT /F1 7 Tf 0.45 0.50 0.56 rg 505 30 Td (${i+1} / ${pageCount}) Tj ET`; return c; });
...
async function generateReportPdf(data){
  try{
    ...
    if(report.organization){
      ...
      const memberById={}; members.forEach(function(m){memberById[String(m.id)]=m;});
      sections.push({title:'Executive Summary',rows:[{type:'kpis',values:[{label:'Members',value:String(members.length)},{label:'Savings',value:formatKes(totalSavings)},{label:'Repayments',value:formatKes(repayments)},{label:'Loans Disbursed',value:formatKes(disbursed)}]}]});
      sections.push({title:'Member Register',rows:[{type:'table',columns:['Member Number','Member Name','Status','Savings'],data:members.slice(0,120).map(m=>[m.unique_member_id||'—',m.full_name||'—',m.is_active?'Active':'Pending',formatKes(m.savings_balance)])}]});
      sections.push({title:'Recent Transaction Log',rows:[{type:'table',columns:['Date','Member Number','Member Name','Type','Amount','Status'],widths:[62,82,128,88,82,69],data:tx.slice(0,120).map(t=>{const m=memberById[String(t.member_id)]||{};return [formatKenyaDate_(new Date(t.created_at)),m.unique_member_id||'—',m.full_name||'Unknown Member',t.type||'—',formatKes(t.amount),t.status||'—'];})}]});
      sections.push({title:'Audit Log',rows:[{type:'table',columns:['Date','Action','Member','Actor','Status'],widths:[65,145,110,110,77],data:audit.slice(0,120).map(a=>[formatKenyaDate_(new Date(a.date)),a.action||'—',a.member?.full_name||a.member?.report_ref||'—',a.actor?.full_name||a.actor?.report_ref||'System',a.status||'—'])}]});
    }else{
      ...
      sections.push({title:'Account Overview',rows:[{type:'identity',items:[{label:'Member Name',value:m.full_name||'Member'},{label:'Member Number',value:m.unique_member_id||'—'},{label:'Account Status',value:m.is_active?'Active':'Pending'}]},{type:'kpis',values:[{label:'Savings Balance',value:formatKes(m.savings_balance)},{label:'Total Savings',value:formatKes(savings)},{label:'Loans Taken',value:formatKes(loaned)},{label:'Repayments',value:formatKes(repayments)}]},]});
